...
Leia-o antes de qualquer mudança. Última atualização: 16/07/2026, fim do dia (pós-Fase 6: refinamentos de UX/produto — ver lista abaixo).
...
- **Identidade visual "bancada de trabalho"** (16/07): fundo verde de cutting
  mat (base #0d5c4d) com grade fina 16px + grade maior 80px — CSS no body
  (home) e dois <Background variant=Lines> no React Flow (acompanham pan/zoom).
  **Paleta** (substitui o §12 dos docs, que era blueprint azul-marinho):
  painel #0f1f1b · card #18342c · ink #eef6f1 · **primary #e8622c (laranja
  "estilete", complementar do verde)** · acentos cyan reservados para IA/
  conhecimento (citações, ghost diff) · semânticos intactos (emerald/amber/red).
  NUNCA usar backdrop-blur em overlays: borra o canvas atrás (bug reportado).
- **Aviso de contexto pendente** aparece SÓ dentro do painel AI Judge — a aba
  lateral "Contexto" é sempre neutra.
- **Propriedades por botão ✎ no próprio nó** (arch e comentário) abrem o card;
  não há mais painel automático por seleção.
...
- **Dock do tutorial**: seta ▾ no canto superior direito minimiza — minimizado
  NUNCA some: vira um pill na base central com "tutorial N/M · título do passo"
  + selo ação/feito (o tutorial não perde o fio); clique/▴ expande. Tem botão
  ← Voltar; o corpo dos passos aceita quebras de linha e identação
  (whitespace-pre-line, max-h-40 com .panel-scroll) — o roteiro em
  tutorial/steps.ts usa bullets identados nos requisitos (intro e contexto).
...
- [x] Refinamento pós-Fase 6 (16/07): tutorial redesenhado para o **feed do
  Twitter/X** (17 passos: hot read path → cache → fan-out on write via fila,
  com Voltar/minimizar e requisitos no intro); Juiz com fixture **genérica**
  (cita REL-005 cache e REL-007 filas — novos docs no corpus-example, release
  2026.07.16); home com hero; fundo cutting-mat; intents genéricos
...
- **Fixture por prompt**: no chat do Arquiteto a pergunta do usuário é a ÚLTIMA
  ...
  A pergunta do tutorial Twitter ("Como escalo a escrita de tweets para milhões
  de seguidores?") tem fixture própria (`architect-17559eca.json`, propõe fila +
  worker de fan-out citando REL-007); "Como reduzo a carga de leitura no banco?"
  → `architect-00d0c2b4.json` (Cache); demais → `architect-default.json`
  (Guardrails — coerente para diagramas com LLM).
